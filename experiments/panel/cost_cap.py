"""Cost-aware cap and AVSL-off, measured on the WF-B protocol.

Earlier diagnostics showed the toxic tail (cost_R > 0.15) is pure round-trip cost
...
Also reports the regime-fold breakdown (per-asset means per fold)
and saves runs/cost_cap.json.

All protocol mechanics (panel loading, fold calendar, ranker, replay,
pooled stats) live in engine.backtest.protocol - this file is only the
...
def main() -> None:
    ...
    print("\n=== cost-cap grid: test pess R (n, dd, panel rows kept) ===",
          flush=True)
    ...
    (REPO / "runs" / f"cost_cap{suffix}.json").write_text(
        json.dumps(results, indent=1))
    print(f"saved runs/cost_cap{suffix}.json", flush=True)
