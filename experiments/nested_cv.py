"""Nested CV on the multi-asset LGBM head (B).
...
the walk-forward baseline.  Discount vs fixed-params B (+0.450R) estimates selection bias.
"""
...
print(f"WF-B reference +0.450R -> external quote: {an.mean():+.3f}R")
...
(REPO / "runs" / "nested_cv.json").write_text(json.dumps(out, indent=1))
print("saved runs/nested_cv.json")
