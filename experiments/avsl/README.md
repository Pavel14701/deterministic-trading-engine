...
### E8 port-check (2026-09-22) — ❌ FAIL, E8 остаётся отложенным

### E8 port-check (2026-09-22) — ❌ FAIL, E8 отложен до engineering

Детектор запускается на 4H с пресетом «4h», геометрия зон sane (ширина 1.35–2.16 ATR, задержки 1–4 бара, S/D сбалансированы), но **9–34 блока на актив за ~7 лет** (BTC: 10) — на 2–3 порядка меньше нужного для EV-теста. Референс: BTC 1H + «1h» даёт 131 блок на 6× больше баров — пресет «4h» (structure filter + complete-window) слишком строг для этой сетки. Следствие: доработка детектора = отдельная задача с замороженными критериями приёмки; тюнить параметры «чтобы запустить» запрещено. Evidence: `runs/port_check_4h.log`.

### OB engineering (2026-09-22) — ✅ ACCEPTANCE PASS, research-пресеты R1–R3

Абляция фильтров (3 раунда, `runs/ob_ablation*.log`): душители блоков — market-structure filter (×2), `min_extreme_gap=8` (×2.3), breakout volume condition (×2.5); prominence/zigzag_distance — no-op для online-зигзага; ADX в «4h» вообще отсутствует; рычаг reversal насыщается при multiple_breakouts; R3 рычаг — фиксированный lookback. Логика детектора не тронута — только пресеты (`experiments/ob/research_presets.py`). Приёмка (`runs/ob_research_check.log`): R1 273–672, R2 575–1146, R3 793–1146 блоков/актив (все 10 активов в [200, 2000]); ширина 1.56–2.09 ATR; задержки 2–3/6–9; S/D 35–56%; регрессия «1h»=131, «4h»=10 точна; детерминизм OK; 46 тестов детектора зелёные.

### E8 (OB-retest 4H, research-пресет R2) — ❌ FAIL, KILL

8538 сделок. **PRIMARY: EV −0.034R (n=5872, z −1.08) против null +0.043±0.027 — 0-й перцентиль**: на довесе 2025 структурные входы были систематически *хуже* случайных баров. F3: +0.084R (n=2666, z +1.93), но 61-й перцентиль — E-a/E-b FAIL; E-c/E-d/E-f проходят, Lift совпадает с режимом E5, но вдвое слабее AVSL и без поддержки гейта. По прегу (провал E-a/E-b) — kill, старый dead-pool вход остаётся мёртвым. Сигнал, проигрывающий случайной геометрии на 5872 сделках, не кандидат на тюнинг. Evidence: `runs/retest_e8.log`.

### Итог re-test семьи (финал)

| Вход | Вердикт |
|---|---|
| z-score (E6) | KILL — режимный lift без holdout-поддержки |
| Donchian (E7) | entry null-confirmed, но риск не чинится сайзингом → overlay CLOSED FINAL |
