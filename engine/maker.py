"""Maker-entry sims + the D.12 maker-entry revisit experiment.

Library part: ``maker_sim`` / ``market_sim`` (signal -> pessimistic R
with limit-fill vs market-entry mechanics).  The D.12 experiment
(formerly ``scripts/d12_maker.py``) runs via ``python -m engine.maker``;
output artifacts keep the d-prefixed filenames in ``runs/`` - see
STATUS.md for history.
...
from engine.sim import COMM, E_MULT, GAP, GEN_SLIP, X_MULT
...
def main() -> None:
    """D.12 maker-entry grid experiment (saves runs/d12_maker.json)."""
