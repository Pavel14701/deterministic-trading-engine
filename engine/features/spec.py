...
Where :mod:`engine.features.events` *discovers* events (bars where a DSL
...
Semantics (same contract as :mod:`engine.features.events` - no look-ahead by
construction):
...
Higher-timeframe features use :func:`engine.features.mtf.asof_join_features`,
...
from engine.features.dsl_feed import SeriesCache, make_bar_context
...
def collect_features(
    ...
    Args:
        ...
        context_factory: optional ``(cache, bar_idx) -> Context`` hook
            ...
            :func:`engine.features.dsl_feed.make_bar_context`.
