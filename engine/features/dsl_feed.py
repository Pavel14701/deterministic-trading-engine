...
:mod:`engine.features.events` (event discovery) and :mod:`engine.features.spec`
...
def make_bar_context(cache: SeriesCache, bar_idx: int) -> Context:
    ...
    :class:`engine.features.provider.ColumnProvider`) via
    ``Context([make_bar_provider(...), extra_provider])`` or the
    :class:`engine.features.provider.HybridContextFactory` hook.
    """
