"""What regime signal separates weak folds (f2, f5) from f1?

The cost-cap experiment located the A-vs-D gap in two folds: 2025-10-27..12-22 and
...
from engine.features.panel import rolling_percentile, trend_state
...
def folds() -> list[tuple[int, int]]:
    """Same fold grid as the cost-cap experiment (ablation-A panels)."""
    tss = []
    for t in TAGS:
        p = pl.read_parquet(
...
def main() -> None:
    ...
    (REPO / "runs" / "regime_diag.json").write_text(json.dumps(
        {"folds": fold_stats, "latency": lat, "triggers": trig_stats},
        indent=1, default=str))
    print("\nsaved runs/regime_diag.json", flush=True)
