...
:func:`engine.mtf.asof_join_features` and other collectors produce
DataFrames whose rows are *aligned 1:1 with the bar frame*.  This
module exposes such columns to DSL expressions (via the
``context_factory`` hook of :func:`engine.feature_spec.collect_features`
and :func:`engine.mfe_mae.collect_mfe_mae`) under strict causal
guards:
...
from engine.dsl_feed import BAR_DSL_MANIFEST, SeriesCache, make_bar_provider
...
class HybridContextFactory:
    ...
    Combines :func:`engine.dsl_feed.make_bar_provider` with a
    ...
    Args:
        extra: Frame row-aligned with the bar frame (e.g. the output
            of :func:`engine.mtf.asof_join_features`).
