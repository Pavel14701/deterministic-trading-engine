...
def sample_batch(
    ...
    """Synthesize a small batch (B=2, T=128) for model testing.

    Direct tensor construction - the old path went through the archived
    torch ``TradingDataset`` (see ``legacy/MANIFEST.md``).
    """
    torch.manual_seed(0)
    seq_len = min(128, len(sample_dataframe))
    tail = sample_dataframe.tail(seq_len)
    prices = np.stack([
        tail["open"].to_numpy(), tail["high"].to_numpy(),
        tail["low"].to_numpy(), tail["close"].to_numpy(),
        tail["volume"].to_numpy().astype(np.float64),
    ], axis=-1).astype(np.float32)  # (T, 5)
    close = prices[:, 3:4]
    batch_prices = np.stack([prices, prices]).astype(np.float32)  # (B, T, 5)
    indicators = np.zeros((2, seq_len, 3), dtype=np.float32)
    signals = np.zeros((2, seq_len, 2), dtype=np.float32)
    tp = np.broadcast_to(close + 2.0, (2, seq_len, 1)).astype(np.float32)
    sl = np.broadcast_to(np.maximum(close - 2.0, 1.0), (2, seq_len, 1))
    action_targets = np.full((2, seq_len), -100, dtype=np.int64)
    outcome_targets = np.full((2, seq_len), 2.0, dtype=np.float32)
    start_indices = torch.arange(2, dtype=torch.int64)
    bar_indices = (torch.arange(seq_len, dtype=torch.int64)
                   .unsqueeze(0).expand(2, -1).contiguous())
    return (
        torch.from_numpy(batch_prices),
        torch.from_numpy(indicators),
        torch.from_numpy(signals),
        torch.from_numpy(tp),
        torch.from_numpy(sl.copy()),
        [list(sample_order_blocks), list(sample_order_blocks)],
        torch.from_numpy(action_targets),
        torch.from_numpy(outcome_targets),
        None,  # pattern_targets: multi-label head is optional
        start_indices,
        bar_indices,
    )
