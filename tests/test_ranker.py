"""Unit tests for the ranker head helpers (engine.model.ranker)."""

import numpy as np
import polars as pl
import pytest

from engine.metrics.trade import (
    bucketed_sharpe,
    per_trade_sharpe,
    trade_curve_stats,
)
...
def test_candidate_key_separates_sides_on_same_bar() -> None:
    from engine.model.ranker import candidate_key
...
def test_trade_curve_stats_dd_depends_on_order() -> None:
    ...
    trade time before pooling (cost-cap/wf experiments pool per asset then fold)."""
