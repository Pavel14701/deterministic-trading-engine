...
## 2026-09-26 — OPTIONS wave-1: A3 FAIL, B2 FAIL, IV-RV timer не валидирован

Карта зарегистрирована (`docs/OPTIONS_MAP.md`), пререги frozen до
кода (`975ed9f`), код v1.0.0 (`3342ed1`), по одному рану на ветку.
Общий движок `_runner.py`: entry по реальным print-IV (+/-1d, медиана;
72% покрытия, как фаза-0), fallback DVOL+frozen skew, long/short
стороны, NOTIONAL 0.10 / HAIRCUT 0.25 как frozen strangle.

**A3 (DVOL-фильтр short strangle, pct>=0.75) — FAIL (G-A1).**
Фильтр отработал структурно: вошло 32/130 legs, IV-RV на вошедших
med +11.8, share 88% (G-A3 PASS), worst leg всего -4.6%, DD 15.1% —
но Sharpe **-1.51**, total -14.8%. Фильтр смягчает безусловный поток
(Sharpe -1.37, total -41.3%, DD 40.7%) примерно вдвое, но знак не
меняет: в q4_hi премия выше, но реализованные хвосты съедают её
полностью. Kill-rule: ветка A3 закрыта, порог 0.75 не пересматривается.

**B2 (risk reversal short put 0.90 / long call 1.10) — FAIL
(G-B1, G-B5).** Sharpe -0.08, total -2.8% за 5.5 лет — экономически
это flat, не edge. Ключевой read-out: put-leg -6.5%, call-leg **-16.2%**
— «дешёвый» call wing после покупки всё равно сгорает тетой; нетто-
кредит медиана -0.13% (реально платим). Skew-премия, измеренная
фазой-0 синхронно, не конвертируется в PnL hold-to-expiry. Kill-rule:
ветка B2 закрыта; вариации страйков — только как новые строки карты.

**IV-RV timer — candidate-check FALSE.** Forward VRP в top-терциле
+9.75pt (порог +5 выполнен), но spread t3-t1 = -1.6pt (< +3):
зависимость не монотонна — после спада спреда forward VRP тоже высок
(mean-reversion IV). Как самостоятельный фильтр таймер закрыт; A3-
механика (уровень DVOL) экономически информативнее спреда, но и она
не проходит гейты.

**Вывод волны 1:** измеренный VRP (+8.8pt синхронно) и skew-структуры
не конвертируются в нетто-положительные hold-to-expiry стратегии на
frozen-инфраструктуре. Все три ветки волны 1 закрыты. Опционный трек
продолжается только там, где меняется механика, а не параметр:
delta-hedged (волна 4, real-print legs) и cross-asset (волна 2,
ETH fetch) — с новыми пререгами. Short-vol без хеджа — закрыто.
## 2026-09-26 — OPTIONS 1.1 delta-hedged short put: FAIL (гамма съедает VRP)

Пререг frozen (`b2bc520`), код v1.0.0 (`02bed84`, движок _hedge
с 5 аналитическими тестами), один ран -> `runs/dh_short_put.log`.

**VERDICT: FAIL (G-H1 Sharpe -0.50, G-H3 leg median/mean < 0).**
45/65 legs hedged (prints 69%), NOTIONAL 0.10, band 0.10, HAIRCUT
0.25. Механика работает идеально: DD 1.2%, worst leg -0.38%
(у unhedged волны-1 worst был на порядок больше). Но:

- total за 5.5 лет **-0.8%** (экон. ноль), по годам плоско.
- Премия на входе была: IV-RV med **+8.5pt**, 78% legs >0.
- Терцили IV-RV vs hedged PnL: t1 +0.03 / t2 -0.05 / t3 -0.00 --
  **связи нет**: даже ноги с самой жирной премией не зарабатывают
  после хеджа.

**Вывод (главный за опционный трек):** измеренный в фазе-0 VRP
(+8.5pt на put-входах) -- это плата за хвост, которая целиком
потребляется дельта-хеджированием (гамма-дренаж + ребалансы).
«VRP есть» и «VRP торгуется» -- разные утверждения; в этой
инфраструктуре верно только первое.

Оговорка (объявлена в пререге): mark-to-IV константен -- влияет
только на daily-поток (Sharpe/DD), НЕ на total PnL (cash-учёт:
премия + хедж-трейды + сеттлмент от IV-пути не зависят). Вердикт
по PnL робастен.

Kill-rule: **1.1 закрыта**. 1.2 (hedged straddle) и 1.3 (var-swap) --
та же механика конверсии IV-RP через гамму; их priors снижаются до
~10% и до нового пререга не открываются (если открываются вообще --
решение в карте). Опционный трек остаётся живым только в
cross-asset (волна 2, фетч идёт) и структурных классах 3/7/8.
## 2026-09-26 — OPTIONS wave-2: ETH-фетч 572/572 + eth_skew read-out (один ран)

Фетч: 572/572 инструментов (238 put / 334 call), 571 ok / 1
corrupt, 327k трейдов (99.8% с IV). Скрипт `eth_skew.py` v1.0.0,
один ран -> `runs/eth_skew.log` (медианы print-IV - DVOL_ETH,
moneyness x year, C/P отдельно).

**Puts:**
- ATM (0.95-1.05): стабильно отрицательно все годы (-5..-11pt) --
  ATM-путы печатаются ДЕШЕВЛЕ DVOL.
- Rich-зона m 0.60-0.75: +10..+15pt, но ТОЛЬКО 2023-2026;
  2021 -- плоско/отрицательно везде, 2022 -- смешанно. Та же
  нестабильность, что у BTC put-skew (2021 негатив).
- Крылья: тонкие + стоxprint-выбросы (+931pt -- стейл).

**Calls:**
- ATM: слегка отрицательно все годы.
- Выше ATM: 2021-2022 отрицательно, 2023-2024 ~0, 2025 -- +12..+31
  на m 1.4-1.7 (upside-премия появилась только в 2025).

**Вывод для B4:** единственная устойчивая rich-зона -- путы
m 0.60-0.75 с 2023 (+10..+15pt), премия мала, режимно зависима и
существует 3 года из 6. После вердикта 1.1 (VRP не конвертируется
в carry) prior B4 заметно снижен. Пререг B4 НЕ замораживается
сейчас: любое его написание -- отдельная строка карты с явным
ответом, почему гамма-дренаж не съест и эту премию, и с заранее
объявленным сплитом 2021-22 vs 2023+. Решение -- за владельцем
карты (см. OPTIONS_MAP).
## 2026-09-26 — OPTIONS wave-3 step 1: term-structure read-out (один ран, без новых данных)

`ts_readout.py` v1.0.0: slope = IV(back,60d) - IV(front,30d) по
frozen-роллам (put m~0.90, оба legs с реальными prints в один
момент). Без фетча, без гейтов -> `runs/ts_readout.log`.

**Результаты:**
- Покрытие тонкое: **22 пары** из ~64 (оба legs с prints +/-1d
  одного roll), по 2-5 пар/год.
- **Устойчивой структуры нет**: медиана slope +1.1pt (0.4 vol-pt
  на базе ~55%), знак нестабилен по годам (2021 +5.5, 2023 -24.6
  на n=2, 2026 -3.4 при 25%>0).
- Предиктивность есть: **spearman(IVf-IVb, RV front-окна) = -0.32**,
  монотонные терцили RV (53.6% -> 40.1% -> 36.3%). Backwardation
  -- индикатор стресса. НО: IVf движется вместе с RV, поэтому VRP
  по терцилям НЕ выше в backwardation (+2.7pt vs +17.2pt) -- сигнал
  не находит ног с жирной премией, наоборот.
- VRP по тенорам: front(30d) +12.8pt vs back(60d) +13.8pt -- тот
  же класс премии, что 1.1 уже показал не конвертируемым.

**Вердикт read-out:** на доступных данных term structure НЕ имеет
устойчивой структуры и не находит VRP, отличный от закрытого в
1.1. Полноценное исследование требует фетча всей цепи (все страйки
x 2-3 экспирации), что при текущих priors не оправдано. Wave 3
замораживается на стадии read-out; календарный пререг НЕ пишется.
Пересмотр -- только после component diagnostic и при появлении
нового механизма, а не тюнинга slope.
## 2026-09-26 — OPTIONS conditional P&L diagnostic: 0 ALIVE -> трек закрыт как класс

Пререг frozen (`427f7f3`), код v1.0.0 (`98f7b12`). Attempt 1:
баг rho_RV=nan (spearman по строкам с NaN RV-окна) -> фикс +
restart, лог attempt1 сохранён. Финальный ран: `runs/cond_pnl_diag.log`.

Юниверсы: U1 unhedged put 65, U2 unhedged call 65, U3 hedged 45.
7 frozen переменных, терцили, правила ALIVE/TRAP/NULL из пререга.

**Результат: 0 ALIVE.** Все 10 оцениваемых ячеек (2 юниверса x 5
переменных с n>=30) -- **TRAP**: rho(X, RV) = +0.34..+0.59 при
spread P&L +0.02..+0.34 (порог ALIVE 0.50). U3 целиком, а также
skew/slope/DTE -- INSUFFICIENT по правилу n>=20 (объявлено в
пререге заранее).

Ключевые цифры:
- put-ноги: выше IV -> P&L лучше (+0.10 -> +0.32), но spread
  0.21 < 0.50 и rho_RV 0.59 -- премия просто отражает RV-режим.
- call-ноги: ВСЕ терцили отрицательны (-0.16..-0.50); выше IV --
  хуже. Short call убыточен безусловно, не только в высоком IV.
- U3 (hedged): INSUFFICIENT по построению (n=45 -> 15/терциль).

**Итоговый вердикт по frozen-правилу пререга: опционный трек
закрыт как класс.** «Благоприятные условия» в доступных
переменных -- vol-proxy: предсказывают realized vol, но IV
движется вместе с ней, и conditional P&L не разделяется. Это
четвёртое независимое подтверждение того же механизма (1.1 unhedged
VRP, wave-3 slope, eth_skew режимность, теперь conditional splits).

Остаётся живым вне опций: component diagnostic (считается,
C1-C6), D1 covered call (deployable beta, не alpha-претензия).
...
## 2026-09-26 (позже) — Дисциплина уровней: B1 кандидат -> CLOSED на cost-гейте

Правка владельца принята: "crush реален" (уровень 1) != "сигнал
торгуем" (уровень 2) != "живой трек" (уровень 3). B1 был на
уровне 1. Порядок гейтов: coverage -> yearly -> cost -> prereg.
Замечание принято: фетч был запущен ДО yearly/cost -- ошибка
порядка; дешёвые гейты всегда первыми.

Прогнаны два гейта, не требующие фетча (`b1_candidate_checks.py`
v1.0.0 -> `runs/b1_candidate_checks.log`):

**Yearly split -- PASS со звёздочкой.** Crush знак-стабилен 5/6
лет: 2021 -2.5pt/59%, 2022 -3.6/86%, 2023 -1.7/86%, 2024
-2.3/86%, 2025 -0.8/65%, 2026 -0.7/50% (медианы, all types).
Не C2-паттерн: не исчезает после 2022, а ослабевает режимно
(низкая IV 2025-26 -> малый абсолютный crush). FOMC-ветка
знак-стабильна все 5 лет.

**Cost gate -- FAIL, B1 ЗАКРЫТ.** gross = 1.51pt x 48.7$/pt x
2 ноги = 147$ (на 1 BTC/ногу) против round-trip 2956$ (30d
ноги, 2x вход/выход x 25% haircut) -- ratio 0.05 при гейте
>=3.0. С 7d ногами 0.10. Разрыв в 60 раз -- не пограничный
случай. Hold-to-expiry (cost = вход 1478$) -- другая стратегия
(short vol с event-таймингом, theta-драйв, gamma-риск), по
правилу "не пробуем другой холд" не рассматривается.

**B1 CLOSED без prereg.** Итоговая карта приложения 2: A1 FAIL,
B1 CLOSED (costs), C1 блокирован (нет историч. OI), C2 CLOSED
(годовой сплит), D1 CLOSED (costs), E1/E2/F1/D2 -- не делаем
(priors/данные), E3 сделано. Опционный трек закрыт полностью;
ни один сигнал не прошёл cost/годовую дисциплину. Открыт только
component diagnostic (идёт). Фетч btc_full (5791) пусть
дофетчивается фоном -- данные останутся в кеше для любых
будущих вопросов, но трека за ними нет.
## 2026-09-26 (финал дня) — Решение владельца: кроем ФОРМУ, не сигнал. Открыта карта B3

Правка принята: "B1 закрыт" было неточно. Точно: **short
strangle/straddle на событие закрыт по costs (0.05x); crush-сигнал
жив** (знак-стабилен 5/6 лет, режимное ослабление -- не C2).
Монетизация -- через другую структуру, которая не платит полный
premium дважды. Открыта карта **B3 Event calendar spread**
(docs/OPTIONS_MAP.md, класс B): short front expiry (пересекает
событие, crush -1.5pt) / long back (crush мал) -- monetizes
differential при меньшем net premium. Это НОВАЯ гипотеза, не
переоткрытие B1 (другой P&L profile: vega-net мал, theta/gamma
иные, pin risk на front; BTC-опционы European -- early exercise
нет).

Дисциплина B3 (жёстче, чем у B1 -- три урока дня):
1. Дешёвые гейты ДО фриза prereg: coverage >= 30 событий с
   prints обеих ног; cost math на ИЗМЕРЕННОМ net premium (заявку
   "20-30% от outright" считаем assumption до замера -- урок D1);
   yearly split по DIFFERENTIAL (не по outright).
2. Любой FAIL = CLOSED, без "другой структуры" (multiple
   comparison) и без "другого холда" (подгонка).
3. Только 1-2-3 PASS -> frozen prereg -> один ран -> первый
   потенциально живой опционный трек; FAIL = финальный вердикт
   "сигнал есть, монетизации нет".

Зависимость: нужны данные back-expiry ATM -- ждём фетч btc_full
