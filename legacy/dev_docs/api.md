...
## 10. Дополнение (сведение с ТЗ)

1. **Единая OHLC-схема**: `open, high, low, close, volume` — канонические имена колонок
   для всех модулей (ta, dsl-провайдер, ai, backtest, white API). Подробнее: TZ-02 п.2.1.
2. **Транспортный слой** очередей white API ↔ локальный GPU-узел описан в TZ-09:
   очереди `md.ohlcv / md.agg / cmd.backtest / cmd.train / evt.report / evt.signals`,
   msgspec-структуры из этого документа используются как язык сообщений через пакет
   `contracts/` (TZ-08). GPU-зависимости (torch) только на локальном узле.
3. **P(win) пересекает границу контуров как число** — модель не выполняется на
   публичном узле; Risk Engine получает готовую вероятность (сквозной принцип TZ-00).
4. `Instrument.tick_sz/lot_sz/min_sz` используются бэктестом для квантования объёма
   (TZ-04 п.4.1.6).
