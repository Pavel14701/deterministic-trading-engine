...
from ta.src.statistics.stdev import (
    stdev_ind,
    stdev_numba,
    stdev_polars,
    stdev_polars_multi,
...
@pytest.mark.statistics
def test_stdev_numba_offset_fillna(
    prices_random_walk: npt.NDArray[np.float64],
) -> None:
    ...
    result_offset = stdev_numba(
        close, length=length, ddof=ddof, offset=1, fillna=0.0
    )
...
def test_stdev_polars_with_offset_fillna(df_random_walk: pl.DataFrame) -> None:
    ...
    close_arr = df_random_walk["close"].to_numpy()
    expected = stdev_numba(
        close_arr, length=length, ddof=1, offset=1, fillna=0.0
    )
    assert_allclose(
        result_series.to_numpy(), expected, rtol=1e-6, equal_nan=True
    )
...
def test_stdev_polars_multi_basic(df_ohlc: pl.DataFrame) -> None:
    ...
    for col in columns:
        ...
        assert_allclose(
            result_df[new_col].to_numpy(), expected, rtol=1e-6, equal_nan=True
        )
