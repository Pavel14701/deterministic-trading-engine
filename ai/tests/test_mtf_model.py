"""Unit tests for the Stage B model-tower helpers (ai.src.mtf_model)."""

import numpy as np
import polars as pl
import pytest

from ai.src.mtf_model import (
...
def test_build_features_columns_and_dtypes() -> None:
    X = build_features(_toy_df())
    assert X.shape[0] == _toy_df().height
    for name in NUMERIC_FEATURES + CATEGORICAL_FEATURES:
        assert name in X.columns
    assert str(X["family"].dtype) == "category"
    assert str(X["atr_pct"].dtype) in ("float32", "float64")
...
def test_candidate_key_separates_sides_on_same_bar() -> None:
    from ai.src.mtf_model import candidate_key
