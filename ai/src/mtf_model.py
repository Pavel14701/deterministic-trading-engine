...
def trade_curve_stats(r_net: np.ndarray) -> dict[str, float]:
    ...
    significance of the mean), max drawdown of the cumulative equity
    curve in R, and the profit factor (gross wins / gross losses).
    """
    r = np.asarray(r_net, dtype=np.float64)
    r = r[np.isfinite(r)]
    if r.size == 0:
...
def per_candidate_pick(
    ...
    for j, key in enumerate(keys):
        idxs = groups[key]
        best_i = max(
            idxs, key=lambda i: float(p_survive[i]) * payoff - (1.0 - p_survive[i])
        )
