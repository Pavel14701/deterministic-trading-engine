...
def bar_duration_ms(target_bar: str) -> int:
    ...
    if target_bar not in BAR_MS:
        msg = f"unsupported bar {target_bar!r}, expected one of {sorted(BAR_MS)}"
        raise ValueError(msg)
...
def resample_ohlcv(df: pl.DataFrame, target_bar: str) -> pl.DataFrame:
    ...
    last_known = int(df["ts"].max()) + _BASE_MS
    out = (
        df.with_columns(pl.from_epoch("ts", time_unit="ms").alias("_dt"))
        .group_by_dynamic("_dt", every=_EVERY[target_bar], closed="left", label="left")
        .agg(
            pl.col("open").first(),
            pl.col("high").max(),
            pl.col("low").min(),
