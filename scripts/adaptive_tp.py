"""D.5: MFE/MAE head -> adaptive take-profit, regime TP rule, RR gates.
(formerly ``scripts/d5_adaptive_tp.py``; output artifacts keep the
...
3. Regime-conditioned TP rule (trend 3R / flat 1R) as the cheap
   non-ML baseline.
Trades = D.4 ranker + table gate picks.  Re-simulated on raw 1H bars
with the D.3 pessimistic cost model (SL-first, slip x2, gap 0.25 ATR).
"""
