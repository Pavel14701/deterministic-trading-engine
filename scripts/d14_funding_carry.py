"""D.14: funding carry feasibility check (first new hypothesis).
...
Outputs runs/d14_funding_carry.json: per-asset funding stats,
...
from engine.marketdata.okx_fetch import fetch_funding_history  # noqa: E402
...
def main() -> None:
    ...
    print("\n=== D.14 funding carry (delta-neutral) ===")
    for key, val in res.items():
        print(f"  {key:22s} {val}")

    (REPO / "runs").mkdir(exist_ok=True)
    (REPO / "runs" / "d14_funding_carry.json").write_text(
        json.dumps(res, indent=1))
    print("saved runs/d14_funding_carry.json", flush=True)
